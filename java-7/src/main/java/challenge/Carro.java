...
public class Carro {
    ...
    private Carro(Motorista motorista, String placa, Cor cor) {
        ...
        this.placa = hasLicensePlate(placa);
        this.cor = hasColor(cor);
    }
    ...
    private String hasLicensePlate(String placa) {
        if (placa == null) {
            throw new NullPointerException("Carro não tem placa.");
        }
        return placa;
    }

    private Cor hasColor(Cor cor) {
        if (cor == null) {
            throw new NullPointerException("Carro não tem cor.");
        }
        return cor;
    }
